...
function getValueDefaultOptions(key: ControlValueKey, logger: IAppLogger): ControlValueAllOptions<any> {
  ...
  if(isCommonControl(key, 'SearchList')) {
    ...
    commonTypeDefaults = { persistent: true, valueConverter: BoleanValueConverter } as ControlValueAllOptions<boolean>;
  ...
  } else if(isSpecificControl(key, [...FindFlightsPageCtrlKey, 'ListView', 'DisplayOptions', 'SecondarySort', 'Dropdown'])) {
  ...
  // Option button group
    ...
    specificControlDefaults = { persistent: false };
  } else if(isSpecificControl(key, [...FindFlightsPageCtrlKey, 'ListView', 'DisplayOptions', 'FlightOffers', 'OptionBtnGroup'])) {
    specificControlDefaults = { 
      defaultValue: getFunctionalElementKey({ sortOption: DefaultFlightOffersSorting, isPrimary: true }) 
    };
  } else if(isSpecificControl(key, [...FindStaysPageCtrlKey, 'ListView', 'DisplayOptions', 'StayOffers', 'OptionBtnGroup'])) {
    specificControlDefaults = { 
  ...
  if(
    isNestedControl(key, [...FindFlightsPageCtrlKey, 'ListView']) ||
    isNestedControl(key, [...FindStaysPageCtrlKey, 'ListView'])
  ) {
    specificControlDefaults ??= { persistent: false };
    specificControlDefaults.persistent = false;
  }
