...
import type { SearchOffersFilterRange, SearchOffersFilterVariantId, ISearchOffersRangeFilterProps, ISearchOffersChecklistFilterProps, ISearchOffersFilterVariant, ISearchStayOffersMainParams, ISearchFlightOffersMainParams, ISearchFlightOffersSortOptions, ISearchOffersCommonSortOptions } from './../types';
...
type ViewFilterProps = ISearchOffersRangeFilterProps | ISearchOffersChecklistFilterProps;
...
function getSortingParamsControlKeys(kind: OfferKind): ControlKey[] {
  if(kind === 'flights') {
    const primary: ControlKey = [...FindFlightsPageCtrlKey, 'ListView', 'DisplayOptions', 'FlightOffers', 'OptionBtnGroup'];
    const secondary: ControlKey = [...FindFlightsPageCtrlKey, 'ListView', 'DisplayOptions', 'FlightOffers', 'Dropdown'];
    return [primary, secondary];
  } else {
    return [
      [...FindStaysPageCtrlKey, 'ListView', 'DisplayOptions', 'StayOffers', 'Dropdown'] as ControlKey
    ];
  }
}
...
function mapSearchOfferRequestParams<TOffersKind extends OfferKind>(
  ...
  if (kind === 'flights') {
    ...
    const resultDto: OffersFetchParamsDto<'flights'> = {
      ...
      ratings: ((searchParams.filters.find(f => f.filterId === FlightsFilterEnum.FlightRating))?.value as SearchOffersFilterVariantId[] | undefined)?.map(v => getRatingFromFilterVariantId(v)),
    ...
    const resultDto: OffersFetchParamsDto<'stays'> = {
      ...
      ratings: (searchParams.filters?.find(f => f.filterId === StaysFilterEnum.StayRating)?.value as SearchOffersFilterVariantId[] | undefined)?.map(v => getRatingFromFilterVariantId(v)),
...
function createSearchFlightOfferFilters(localizer: Localizer, logger: IAppLogger): (ISearchOffersRangeFilterProps | ISearchOffersChecklistFilterProps)[] {
...
function createSearchStayOfferFilters(localizer: Localizer, logger: IAppLogger): (ISearchOffersRangeFilterProps | ISearchOffersChecklistFilterProps)[] {
