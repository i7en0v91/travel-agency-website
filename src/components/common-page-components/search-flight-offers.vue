...
if (props.takeInitialValuesFromUrlQuery) {
  searchOffersStore = await searchOffersStoreAccessor.getInstance('flights', true);
  displayedSearchParams = computed<Partial<ISearchFlightOffersMainParams>>(() => { return searchOffersStore!.currentSearchParams; });
} else {
  searchOffersStore = await searchOffersStoreAccessor.getInstance('flights', false);
  displayedSearchParams = computed<Partial<ISearchFlightOffersMainParams>>(() => {
    return {
      type: 'flights',
...
watch(displayedSearchParams, () => {
  logger.verbose(`(SearchFlightOffers) firing change event, ctrlKey=${props.ctrlKey}, params=${JSON.stringify(displayedSearchParams!.value)}`);
