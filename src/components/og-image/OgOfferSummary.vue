  ...
  <div :class="`container`">
    <NuxtErrorBoundary @error="onError" :class="CommonClass">
      <div :class="`og-app-page ${CommonClass}`">
        ...
        <div :class="`og-offer-summary ${CommonClass}`">
          <h4 :class="`offer-title font-header color-primary ${CommonClass}`">
            {{ getLocalizeableValue(title, locale as Locale) }}
          </h4>
          <div :class="`offer-location font-body color-secondary ${CommonClass}`">
            <svg
              :class="`location-icon ${CommonClass}`"
              width="10"
              height="14"
              viewBox="0 0 10 14"
              fill="none"
            ...
            <div :class="`offer-location-text ${CommonClass}`">
              {{ getLocalizeableValue(city.country.name, locale as Locale) }}, {{ getLocalizeableValue(city.name, locale as Locale) }}
            </div>
          </div>
        </div>
        <div v-if="kind === 'flights'" :class="`offer-price font-body font-bold color-highlight ${CommonClass}`">
          {{ Math.floor(price) }}$
        </div>
        <div v-else :class="`offer-price font-body font-bold color-highlight ${CommonClass}`">
          {{ Math.floor(price) }}$&#47;{{ $t(getI18nResName2('searchStays', 'night')) }}
        </div>
        <div :class="`offer-date font-body font-bold ${CommonClass}`">
          {{ dateStr }}
        </div>
        <nuxt-img
          :src="imgUrl"
