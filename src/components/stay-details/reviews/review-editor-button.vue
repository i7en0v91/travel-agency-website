...
<template>
  <UButton :icon="iconsMap[props.type]" size="lg" :class="`flex-initial text-gray-500 dark:text-gray-400 focus-visible:ring-2 focus-visible:ring-gray-500 dark:focus-visible:ring-gray-400 bg-transparent dark:bg-transparent hover:bg-gray-200 dark:hover:bg-gray-800 ${props.active ? 'bg-primary-200 dark:bg-primary-700 hover:bg-primary-300 dark:hover:bg-primary-800' : ''} disabled:!bg-transparent aria-disabled:!bg-transparent dark:disabled:!bg-transparent dark:aria-disabled:!bg-transparent disabled:opacity-50 dark:disabled:opacity-50`" variant="soft" color="gray" :disabled="disabled" :aria-label="$t(getI18nResName3('reviewEditor', 'buttons', type))" :title="t(getI18nResName3('reviewEditor', 'buttons', type))" @click="onClick"/>
</template>
