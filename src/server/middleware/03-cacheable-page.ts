...
import { type CacheByPageTimestamp, type EntityId, type IHtmlPageCacheCleaner, type PageCacheVaryOptions } from '../../shared/interfaces';
import { getPagePath, SystemPage, type AppPage } from '../../shared/page-query-params';
...
async function handleMissedParams(
  ...
  if(cacheVaryOptions === 'UseEntityChangeTimestamp' && unresolvedParams.length === 1 && unresolvedParams[0] === QueryPageTimestampParam) {
    ...
    if(isCachingEnabled) {
      ...
      redirectQuery = set(redirectQuery, QueryPageTimestampParam, isDate(pageTimestamp) ? pageTimestamp.getTime() : (pageTimestamp ?? <UninitializedPageTimestamp>0)?.toString());
      queryChanged = true;
    }
