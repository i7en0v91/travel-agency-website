...
export class AcsysClientBase implements IAcsysClientBase, IAcsysAuthState { 
  ...
  protected readonly userOptions: IUserOptions;
  ...
  constructor (baseUrl: string, userOptions: IUserOptions, roleKind: UserRoleEnum, logger: IAppLogger) {
    this.logger = logger;
    ...
    (fullPath: string, query: any, body: any, method: HTTPMethod, minimumUserRole: UserRoleEnum | undefined, withAuth: boolean | 'accept-pending-status', respType: ApiResponseTypes, bodyValidationSchema?: TReqSchema): Promise<TRes> => 
  {
    ...
    if(body) {
      if(!(body instanceof FormData) && bodyValidationSchema) {
        ...
        const validationError = validateObject(body, bodyValidationSchema);
        if (validationError) {
    ...
    const parseBytesResponse = async (response: Response): Promise<FetchFileResponse> => {
      ...
      try {
        ...
        this.logger.warn(`(AcsysClientBase) [${this.roleKind}] failed to parse file last modified time, value=${lastModifiedUtcStr}, path=${fullPath}, method=${method}, minRole=${minimumUserRole}, responseStatus=${response.status}, responseStatusText=${response.statusText}`, (isObject(body) && !(body instanceof FormData)) ? { query, body } : { query });
