import type { IImageData, PreviewMode, IImageLogic, IAppLogger, EntityId, IAuthFormImageLogic, AuthFormImageData, IAuthFormImageInfo } from '../app-facade/interfaces';
import { ImageCategory, newUniqueId, DbVersionInitial } from '../app-facade/implementation';
...
export class AuthFormImageLogic implements IAuthFormImageLogic {
  ...
  private async createWithImageId(imageId: EntityId, order: number, event: H3Event<EventHandlerRequest>, previewMode: PreviewMode): Promise<string> {
    ...
    if(previewMode) {
      ...
      resultId = await this.prismaImplementation.createImage(imageId, order, event, previewMode);
    }
    return resultId;
  };

  private async createFromImageData(imageData: AuthFormImageData, order: number, event: H3Event<EventHandlerRequest>, previewMode: PreviewMode): Promise<string> {
    let resultId: EntityId;
    if(previewMode) {
      resultId = await this.dbRepository.$transaction(async () => {
        ...
        const image = (await this.imageLogic.createImage(imgParams, undefined, event, previewMode));
      ...
      resultId = await this.prismaImplementation.createImage(imageData, order, event, previewMode);
    }
    return resultId;
  };

  async createImage(imageData: EntityId | AuthFormImageData, order: number, event: H3Event<EventHandlerRequest>, previewMode: PreviewMode): Promise<string> {
    this.logger.debug(`(AuthFormImageLogic-Acsys) creating auth form image, order=${order}, previewMode=${previewMode}`, imageData);
    const resultId = isString(imageData) ? this.createWithImageId(imageData, order, event, previewMode) : this.createFromImageData(imageData, order, event, previewMode);
