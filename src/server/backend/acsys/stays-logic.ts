import { type ReviewSummary, type PreviewMode, type IAppLogger, type IStayOfferDetails, type IStayReview, type StayServiceLevel, type ISearchStayOffersResult, type StayOffersSortFactor, type IStayOffersFilterParams, type IStaysLogic, type IPagination, type ISorting, type EntityId, type IStayShort, type EntityDataAttrsOnly, type IStayOffer, type IStay, type IStayData, DefaultStayReviewScore } from './../../backend/app-facade/interfaces';
...
export class StaysLogic implements IStaysLogic {
  ...
  async getAllStays (previewMode: PreviewMode): Promise<IStayShort[]> {
    ...
    if(previewMode) {
      const stayResolveResult = [...(await this.acsysDraftsEntitiesResolver.resolveStays({})).items.values()];
