import { type PreviewMode, type IAppLogger, type IAirplane, type EntityId, type IAirplaneLogic, type IAirplaneData } from './../../backend/app-facade/interfaces';
...
export class AirplaneLogic implements IAirplaneLogic {
  ...
  async getAllAirplanes (allowCachedValue: boolean, previewMode: PreviewMode): Promise<IAirplane[]> {
    ...
    let result: IAirplane[] | undefined;
    if(previewMode) {
      result = [...(await this.acsysDraftsEntitiesResolver.resolveAirplanes({})).items.values()];
    } else {
