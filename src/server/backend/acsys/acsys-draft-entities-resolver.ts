import { type IStayOfferDetails, type IStayDescription, type IStayImage, type IStay, type IFlight, type ICompanyReview, type IAuthFormImageInfo, type IAirport, type AirplaneImageKind, type ICity, type IFileLogic, type IAppLogger, type IEntity, type ILocalizableValue, type EntityId, type IAirlineCompany, type IImageInfo, type IFileInfo, type IImageCategoryInfo, ImageCategory, type ICountry, type IAirplane, type IAirplaneImage, type IFlightOffer, type FlightClass, type EntityDataAttrsOnly, type StayServiceLevel, type IStayImageShort, type StayDescriptionParagraphType, DefaultStayReviewScore } from '../app-facade/interfaces';
import { AppException, AppExceptionCodeEnum, lookupValueOrThrow } from '../app-facade/implementation';
import { mapDbDate, mapDbGeoCoord } from '../../../server/backend/helpers/db';
...
export class AcsysDraftEntitiesResolver {
  ...
  private formatResolveResultLog<TEntity extends IEntity>(options: AcsysDraftEntitiesResolveOptions, result: AcsysDraftEntitiesResolveResult<TEntity>): string {
    return `count=${result.items.size} of ${options.idsFilter ? options.idsFilter.length : 'all'} (${result.notFoundIds?.length ? `NOT FOUND ids=[${result.notFoundIds.join('; ')}]; `: ''}ids=[${[...result.items.values()].map(x => x.id).join('; ')}])`;
  }
  ...
  public async resolveAirplanes(options: AcsysDraftEntitiesResolveOptions): Promise<AcsysDraftEntitiesResolveResult<IAirplane>> {
    ...
    const resolvedAirplaneImages = new Map(toPairs(groupBy(
      [...(await this.resolveAirplaneImages(this.buildNestedEntitiesResolveOptions(airplaneIds, options))).items.values()], 
      i => i.airplaneId
    )));
    ...
    for(let i = 0; i < allUnresolved.length; i++) {
      ...
      const images = resolvedAirplaneImages.get(unresolved.id);
  ...
  public async resolveStays(options: AcsysDraftEntitiesResolveOptions): Promise<AcsysDraftEntitiesResolveResult<IStay>> {
    ...
    const resolvedStayImages = new Map(toPairs(groupBy(
      [...(await this.resolveStayImages(this.buildNestedEntitiesResolveOptions(stayIds, options))).items.values()], 
      i => i.hotelId
    )));
    this.logger.debug(`(AcsysDraftEntitiesResolver) resolving stays - stay descriptions`);
    const resolvedStayDescriptions = new Map(toPairs(groupBy(
      [...(await this.resolveStayDescriptions(this.buildNestedEntitiesResolveOptions(stayIds, options))).items.values()], 
      i => i.hotelId
    )));
    ...
    for(let i = 0; i < allUnresolved.length; i++) {
      ...
      const images = resolvedStayImages.get(unresolved.id);
      ...
      const description = resolvedStayDescriptions.get(unresolved.id);
