import { type PreviewMode, type IAppLogger, type ICityShort, type ICityData, type ICountry, type ICountryData, type IGeoLogic, type EntityId, type DistanceUnitKm } from './../app-facade/interfaces';
import { newUniqueId, DbVersionInitial } from './../app-facade/implementation';
...
export class GeoLogic implements IGeoLogic {
  ...
  async getAllCities (previewMode: PreviewMode): Promise<ICityShort[]> {
    ...
    if(previewMode) {
      result = [...(await this.acsysDraftsEntitiesResolver.resolveCities({})).items.values()].map(city => {
        return <ICityShort>{
