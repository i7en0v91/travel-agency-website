import type { IFileLogic, IImageCategoryLogic, PreviewMode, IImageFileInfoUnresolved, IAppLogger, ImageCategory, IImageBytes, EntityId, IImageInfo, IImageLogic, IImageData, Timestamp, ImageCheckAccessResult } from '../app-facade/interfaces';
import { DbVersionInitial, newUniqueId, CachedResultsInAppServicesEnabled } from '../app-facade/implementation';
...
export class ImageLogic implements IImageLogic {
  ...
  async createImage (data: IImageData, userId: EntityId | undefined, event: H3Event, previewMode: PreviewMode): Promise<{ id: EntityId, timestamp: Timestamp }> {
    ...
    if(previewMode) {
      ...
      result = await this.prismaImplementation.createImage(data, userId, event, previewMode);
    }
  ...
  async findImage (id: EntityId | undefined, slug: string | undefined, category: ImageCategory, event: H3Event, previewMode: PreviewMode): Promise<IImageInfo | undefined> {
    ...
    if(previewMode) {
      ...
      if (queryResult) {
        ...
        const fileInfos = [...fileInfoResultResult.items.values()];
        if (!fileInfos?.length) {
