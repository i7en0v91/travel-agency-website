import { type PreviewMode, type ICitiesSearchQuery, type IAppLogger, type EntityId, type ICitiesLogic, type ICitySearchItem, type ICity, type IPopularCityData, type IPopularCityItem, type ITravelDetails } from './../../backend/app-facade/interfaces';
import { DbVersionInitial, newUniqueId, AppException, AppExceptionCodeEnum } from './../../backend/app-facade/implementation';
...
export class CitiesLogic implements ICitiesLogic {
  ...
  async getCity (slug: string, previewMode: PreviewMode): Promise<ICity> {
    ...
    if(previewMode) {
      ...
      const resolveResult = [...(await this.acsysDraftsEntitiesResolver.resolveCities({ idsFilter: [cityId] })).items.values()];
  ...
  async getTravelDetails (cityId: EntityId, previewMode: PreviewMode): Promise<Omit<ITravelDetails, 'price'>> {
    ...
    if(previewMode) {
      ...
      if (popularCityEntity) {
        ...
        const city = [...resolvedCity.items][0][1];
        ...
        const images = orderBy([...resolvedImages.items.values()], v => popularCityImageInfos.find(i => i.imageId === v.id)!.orderNum).map((i) => { return { slug: i.slug, timestamp: i.file.modifiedUtc.getTime() }; });
  ...
  async getPopularCities (previewMode: PreviewMode): Promise<IPopularCityItem[]> {
    ...
    if(previewMode) {
      ...
      const resolvedImages = await this.acsysDraftsEntitiesResolver.resolveImageFileInfos({ idsFilter: [...cityImageIdsMap.values()] });

      this.logger.debug(`(CitiesLogic-Acsys) listing popular cities - mapping result entities, previewMode=${previewMode}`);
      result = [];
      const cities = [...resolvedCities.items.values()];
