...
export class UserLogic implements IUserLogic {
  ...
  updateUserAccount = async (userId: EntityId, firstName: string | undefined, lastName: string | undefined, password: string | undefined, emails: string[] | undefined, theme: Theme | undefined, locale: Locale | undefined, event: H3Event): Promise<UpdateUserAccountResult> => {
    ...
    const userInfo = await this.getUser(userId, 'minimal', event);
    if (!userInfo) {
  ...
  verifyUserPassword = async (email: string, password: string): Promise<IUserMinimalInfo | undefined> => {
    ...
    const userEntity = await this.findUserByEmail(email, true, 'profile', undefined);
    if (!userEntity) {
  ...
  registerUserByEmail = async (email: string, password: string, verification: RegisterVerificationFlow, firstName: string | undefined, lastName: string | undefined, theme: Theme | undefined, locale: Locale | undefined, event: H3Event): Promise<RegisterUserByEmailResponse> => {
    ...
    const existingUser = await this.findUserByEmail(email, false, 'minimal', event);
    if (existingUser) {
  ...
  resolveUserFiles = async (userProfileUnresolved: IUserProfileFileInfoUnresolved, event: H3Event | undefined): Promise<IUserProfileInfo> => {
  ...
  getUser = async <TDataSet extends keyof UserResponseDataSet>(userId: EntityId, dataSet: TDataSet, event: H3Event): Promise<UserResponseDataSet[TDataSet] | undefined> => {
    ...
    if (dataSet === 'profile') {
      ...
      const result = await this.resolveUserFiles(userProfileUnresolved, event);
  ...
  findUserByEmail = async <TDataSet extends keyof UserResponseDataSet>(
    email: string, mustBeVerified: boolean, dataSet: TDataSet, event: H3Event | undefined): Promise<UserResponseDataSet[TDataSet] | undefined> => {
  ...
  ensureOAuthUser = async (authProvider: AuthProvider, providerIdentity: string, firstName: string | undefined, lastName: string | undefined, email: string | undefined, emailVerified: boolean | undefined): Promise<IUserProfileInfo> => {
    ...
    if (userEntity) {
      const userProfileUnresolved = MapUserEntityProfile(userEntity);
      const result = await this.resolveUserFiles(userProfileUnresolved, undefined);
  ...
  recoverUserPassword = async (email: string, theme: Theme | undefined, locale: Locale | undefined, event: H3Event): Promise<PasswordRecoveryResult> => {
    ...
    try {
      const userEntity = await this.findUserByEmail(email, false, 'minimal', event);
      if (!userEntity) {
