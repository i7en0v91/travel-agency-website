...
import { ChangeDependencyTracker, type EntityModel, type IChangeDependencyTracker } from './../common-services/change-dependency-tracker';
import { createLogger } from './../helpers/testing';
import { newUniqueId,AvailableLocaleCodes, DbVersionInitial, HtmlPageTimestampTable, type IAppLogger, AppConfig, TokenKind, type EntityId, AuthProvider, ImageCategory, type FlightClass, type StayDescriptionParagraphType, type StayServiceLevel, EmailTemplateEnum } from '@golobe-demo/shared';
import { createPrismaClient } from './../prisma/client';
import { getSomeSalt, calculatePasswordHash } from './../helpers/utils';
import { generateNewTokenValue } from './../helpers/tokens';
...
export const AllEntityModels: EntityModel[] = DomainModelDefinition.datamodel.models.filter(m => m.name !== HtmlPageTimestampTable && !m.name.startsWith(AppConfig.acsys.draftsEntityPrefix)).map(m => m.name as EntityModel);
...
describe('unit:change-dependency-tracker tracking changes in related entities when a change occurs in any entity', async () => {
  ...
  async function createImageCategoryEntityTestCase(testWithDeleted: boolean): Promise<ITestCase> {
    ...
    const imageIds = range(0, 4).map(() => newUniqueId());
