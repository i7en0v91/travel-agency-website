...
function buildSqliteViewStatements(sourceCollection: SourceCollection, viewConfig: ViewConfig): string {
  ...
  if(viewConfig.draftsAllowed) {
    sql += buildViewWithTriggersSqlStatements(`acsys_${viewName}`, columnSettings, `${AcsysDraftsEntityPrefix}${sourceTableName}`);
  } else {
...
function buildMySqlViewStatements(sourceCollection: SourceCollection, viewConfig: ViewConfig): string {
  ...
  if(viewConfig.draftsAllowed) {
    sql += buildViewWithTriggersSqlStatements(`acsys_${viewName}`, columnNamesList, `${AcsysDraftsEntityPrefix}${sourceTableName}`, setNullIfEmptyColumns);
  } else {
