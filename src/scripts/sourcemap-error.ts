...
async function run () {
  consola.log('==== SOURCEMAPPING ERROR STACK ====');
  consola.log(`source map directory: ${sourceMapsDir}`);
  consola.log(`error stack: ${errorStack}`);

  const parsedStack: IStackLine[] = [];

  const errorLines = errorStack.split('\r').flatMap((v, _) => v.split('\n')).filter((v, _) => v.trim().length > 0);
  consola.log(`error lines count: ${errorLines.length}`);
  if (errorLines.length === 0) {
    consola.log('stack is empty, exit');
    process.exit(0);
  }
  ...
  for (let i = 0; i < errorLines.length; i++) {
    ...
    if (tokens.length !== 2) {
      consola.log(`line #${i + 1} has to contain exactly 1 '?' symbol, aborting`);
      process.exit(1);
    }
    ...
    if (tokens.length !== 2) {
      consola.log(`line #${i + 1} has to contain exactly 1 ':' symbol in symbol location part, aborting`);
      process.exit(1);
    }
    ...
    consola.log(`parsed line: file='${fileName}'; lineNumber=${lineNumber}; columnNumber=${columnNumber}`);
  }
  ...
  for (let i = 0; i < parsedStack.length; i++) {
    ...
    if (!fs.existsSync(sourceMapFile)) {
      consola.log(`source map not found for line# ${trace.order}: ${sourceMapFile}, aborting`);
      process.exit(1);
    }

    const mapJson = fs.readFileSync(sourceMapFile, 'utf8');
    consola.log(trace.column);
    const map = destr(mapJson) as any;
    const sm = await new smc.SourceMapConsumer(map);
    consola.log(sm.originalPositionFor(trace));
    sm.destroy();
  }
}
